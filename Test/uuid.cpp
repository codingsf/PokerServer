...
BOOST_AUTO_TEST_CASE(getUuid)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		vecUuid.push_back("111111");
	}
	std::cout << "生成uuid用时:" << t.elapsed() << std::endl;
}

extern int done;
void OnGetUuid(SessionPtr pSes, std::string uuid, boost::shared_future<msgpack::object> fut)
{
	try
	{
		BOOST_CHECK_EQUAL(fut.get().as<std::string>(), uuid);
		done++;
	}
...
void OnSetUuid(SessionPtr pSes, std::string uuid, boost::shared_future<msgpack::object> fut)
{
	try
	{
...
{
	std::cout << "BGN 异步set/getUuid" << std::endl;
	...
	try
	{
		...
		client_io.run();
		std::cout << "		" << t.elapsed() << "秒		成功: " << done << "次" << std::endl;
	}
	catch (const std::exception& e)
	{
		std::cerr << "call failed: " << e.what() << std::endl;
	}
	std::cout << "END 异步set/getUuid" << std::endl << std::endl;
}

BOOST_AUTO_TEST_CASE(uuidSyncCall)
{
	std::cout << "BGN 同步set/getUuid" << std::endl;
	...
	try
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			BOOST_CHECK_EQUAL(fut2.get().as<std::string>(), vecUuid[i]);
		}
	...
	client_io.stop();
	clinet_thread.join();
	std::cout << "END 同步set/getUuid" << std::endl << std::endl;
}
